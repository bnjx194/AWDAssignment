    ...
    <section class="profile-card profile-activity-card">
        @if($user->role === 'seller')
            ...
            @if($listedProperties->isEmpty())
                <p class="empty-state">You have not listed any properties yet.</p>
            @else
                <div class="activity-list">
                    @foreach($listedProperties as $property)
                        <article class="activity-item">
                            <div>
                                <h3>{{ Str::limit($property->description, 60) }}</h3>
                                <p>
                            ...
                            <div class="activity-meta">
                                <span class="badge">{{ optional($property->listing)->status ?? 'unlisted' }}</span>
                                @if($property->listing && $property->listing->price)
                                    <span>RM {{ number_format($property->listing->price, 2) }}</span>
                                @endif
        ...
        @elseif($user->role === 'buyer')
            ...
            @if($purchasedTransactions->isEmpty())
                <p class="empty-state">You have not purchased any properties yet.</p>
            @else
                <div class="activity-list">
                    @foreach($purchasedTransactions as $transaction)

                        {{-- Swapped out the PHP 8 nullsafe operator for Laravel's optional() --}}
                        @php
                        ...
                        <article class="activity-item">
                            <div>
                                <h3>{{ $property ? Str::limit($property->description, 60) : 'Property unavailable' }}</h3>
                                <p>
                                    {{ optional(optional($property)->address)->address ?? 'Address unavailable' }}
                                    @if($property && $property->address && $property->address->postal_code)
                                        , {{ $property->address->postal_code }}
                                    @endif
                                </p>
                            </div>
                            ...
                            <div class="activity-meta">
                                ...
                                <span>RM {{ number_format($transaction->final_price, 2) }}</span>
                                @if($property)
                                    <a href="{{ route('property.show', $property->id) }}">View</a>
                                @endif
                            </div>
